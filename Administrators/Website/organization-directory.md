...
A searchable, browsable public directory of the organizations in your diocese — parishes, schools, cemeteries, hospitals, the chancery, or any mix of them. Visitors can search by name or city, browse alphabetically (with a letter bar to jump straight to a section), by city, or by deanery/region (if you use those), filter down to one city, or search by distance from a ZIP code. A map showing every result sits alongside the list at all times; each result also has its own "Map," "Directions," and "Details" links. No sign-in required.
...
| `browse-group-type-id` | Adds a "By [group]" toggle alongside the default A–Z, By City, and Nearest options, grouping results by Location Group (e.g. deanery). Find the ID on the **Location Group Types** page. Leave it off to show only A–Z / By City / Nearest. | `browse-group-type-id="1"` |
...
| `show-address` / `show-phone` / `show-description` | Toggle these optional pieces of information on each result. | `show-phone="false"` |
...
- A "Filter by city" dropdown appears automatically whenever the results span more than one city — no attribute needed. It narrows the list to one city and composes with whatever Browse option (A–Z, By City, By Group, Nearest) and search text are active.
- Choosing **A–Z** shows a letter bar above the results; clicking a letter jumps straight to that section (loading more results first if needed), and a letter with no matches is shown disabled. Choosing **Nearest** reveals the ZIP-code / "Use my location" panel — it stays hidden the rest of the time so it doesn't clutter the page.
...
- On a wide screen, the map stays in view (sticks 20px below the top of the browser window) while you scroll a long results list. If your site has its own sticky/fixed header, the map can end up scrolling partly underneath it — set `--map-sticky-top` to that header's height (plus a little breathing room) via [`customcss`](custom-styling.md), e.g. `:host { --map-sticky-top: 110px; }`.
