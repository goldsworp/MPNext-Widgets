 ...
 *   pnpm run setup:check   # Validation-only mode
 */
...
function checkMPTypes(): StepResult {
  ...
  return {
    ...
    details: 'Run pnpm run mp:generate:models to generate types',
  };
}
...
function checkBuildCache(): StepResult {
  ...
  return {
    ...
    details: 'Run pnpm run build to create a production build',
  };
}
...
async function runInteractiveSetup(options: SetupOptions): Promise<number> {
  ...
  if (options.skipInstall) {
    ...
    const installResult = await execCommandStreaming('pnpm', ['install'], options.verbose);
  ...
  if (options.skipInstall) {
    ...
    console.log(chalk.gray('  Running pnpm update...'));
    const updateResult = await execCommandStreaming('pnpm', ['update'], options.verbose);

    if (updateResult.success) {
      console.log(chalk.green('  ✓ Dependencies updated'));
      passedSteps++;
    } else {
      console.log(chalk.yellow('  ⚠ pnpm update had issues (non-critical)'));
      warnings++;
      passedSteps++;
    }
  }
  ...
  console.log(chalk.gray('  Running pnpm run build...'));

  const buildResult = await execCommandStreaming('pnpm', ['run', 'build'], options.verbose);
